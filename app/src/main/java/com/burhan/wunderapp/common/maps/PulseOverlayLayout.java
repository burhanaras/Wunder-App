...
public class PulseOverlayLayout extends MapOverlayLayout {
    ...
    public void setupMarkers(final Point point, final LatLng latLng) {
        startMarker = new PulseMarkerView(getContext(), latLng, point, true);
        finishMarker = new PulseMarkerView(getContext(), latLng, point, true);
    }
    ...
    private PulseMarkerView createPulseMarkerView(final LatLng latLng) {
        return new PulseMarkerView(getContext(), latLng, googleMap.getProjection().toScreenLocation(latLng),true);
    }
    ...
    public void createAndShowMarker(final int position, final LatLng latLng) {
        ...
        marker.setOnClickListener(new MarkerClickListener(this, position));
    }

    public void createAndShowGPSMarker(final LatLng latLng) {
        PulseMarkerView marker = createPulseMarkerView(latLng);
        marker.showWithDelay(scaleAnimationDelay);
        scaleAnimationDelay += ANIMATION_DELAY_FACTOR;

        marker.updatePulseViewLayoutParams(googleMap.getProjection().toScreenLocation(latLng));
        addMarker(marker);
        marker.show();
